...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) setQualityOpen(false);
    };
  ...
  return (
    <header className="flex h-12 shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-4 dark:border-gray-700 dark:bg-gray-800">
      {/* Logo */}
      <div className="flex items-center gap-2">
        <h1 className="text-lg font-bold tracking-tight text-gray-900 dark:text-white">Draw It</h1>
      ...
      {/* Auto/Manual toggle (for HD auto-trigger) */}
      {hasOpenAI && (
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={toggleAutoGenerate} role="switch" aria-checked={autoGenerate}
            className={`relative h-5 w-9 rounded-full transition-colors ${autoGenerate ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
            <span className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${autoGenerate ? 'translate-x-4' : ''}`} />
          </button>
          <span className={`text-[11px] ${autoGenerate ? 'font-medium text-gray-700 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500'}`}>Auto HD</span>
        </div>
      )}

      {/* Dark mode */}
      <button onClick={toggleTheme} aria-label="Toggle theme"
        className="flex h-7 w-7 items-center justify-center rounded-md text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700">
        {theme === 'light' ? (
      ...
      {hasOpenAI && (
        <div className="relative" ref={dropdownRef}>
          <button onClick={() => setQualityOpen(!qualityOpen)}
            className="flex items-center gap-1 rounded-md bg-gray-100 px-2 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
            HD: {currentQuality?.label}
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-3 w-3"><path d="m6 9 6 6 6-6" /></svg>
          </button>
          {qualityOpen && (
            <div className="absolute top-full left-0 z-40 mt-1 w-40 rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-600 dark:bg-gray-700">
              {QUALITY_OPTIONS.map((q) => (
                <button key={q.value} onClick={() => handleQualitySelect(q.value)}
                  className={`flex w-full items-center justify-between px-3 py-1.5 text-xs ${
                    selectedQuality === q.value ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300'
                      : 'text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-600'
                  }`}>
                  <span>{q.label}</span><span className="text-gray-400 dark:text-gray-500">{q.cost}</span>
                </button>
              ))}
            </div>
          )}
